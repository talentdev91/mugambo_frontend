...
import { deleteToken, addedTokenInfos } from '../../../../../store/actions/token'
...
interface CustomizedTableProps {
  ...
  addedTokenInfos: (idArr: any) => void
...
const Table1: React.FC<CustomizedTableProps> = ({
  rows,
  deleteToken,
  handleClickOpen,
  addedTokenInfos,
  ...
  // useEffect(() => {
  //   gridData.resolve()
  // }, [gridData, addedTokenInfos])

  const deleteTk = (id: any) => {
    deleteToken(id)
  }
  const addTk = (idArr: any) => {
    addedTokenInfos(idArr)
  }
  return (
    <>
      ...
      <MaterialTable
        ...
        actions={[
          {
            tooltip: 'Added Real TokenInfomation Table All Selected TokenInfos',
            icon: 'add',
            onClick: (evt, data: any) => {
              addTk(data.map((a: any) => a._id))
            },
          },
        ]}
      />
...
export default connect(null, { deleteToken, addedTokenInfos })(Table1)
